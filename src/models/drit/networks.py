# Disgracefully stolen from https://github.com/HsinYingLee/DRIT/blob/master/src/networks.py
# With slight modification (disable wight sharing)
...
class G_concat(nn.Module):
  def __init__(self, output_dim_a, output_dim_b, nz):
    ...
    decA0 = []
    decA0 += [INSResBlock(tch, tch)]
    self.decA0 = nn.Sequential(*decA0)

    decB0 = []
    decB0 += [INSResBlock(tch, tch)]
    self.decB0 = nn.Sequential(*decB0)
