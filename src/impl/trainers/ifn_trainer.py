...
class IFNTrainer(CDTrainer):
    ...
    def train_epoch(self, epoch):
        ...
        for i, (t1, t2, tar) in enumerate(pb):
            ...
            all_losses = [self.criterion(F.interpolate(pred, tar.shape[1:]).squeeze(1), tar) for pred in preds]
            # All weights set to 1
            loss = sum(all_losses)
            loss_final = all_losses[0]
            
            losses.update(loss.item(), n=self.batch_size)
            losses_final.update(loss_final.item(), n=self.batch_size)

            self.optimizer.zero_grad()
            loss.backward()
            self.optimizer.step()

            desc = (start_pattern+" Loss: {:.4f} ({:.4f}) Loss_final: {:.4f} ({:.4f})").format(
                i+1, len_train, 
                losses.val, losses.avg,
                losses_final.val, losses_final.avg
            )
            ...
            if self.tb_on:
                ...
                self.tb_writer.add_scalar("Train/loss_final", losses_final.val, self.train_step)
                if show_imgs_on_tb:
    ...
    def evaluate_epoch(self, epoch):
        ...
        if self.tb_on:
            ...
            self.tb_writer.add_scalar("Eval/loss_final", losses_final.avg, self.eval_step)
